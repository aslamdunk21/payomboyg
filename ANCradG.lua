...
-- [FIX] หยุดการทำงานของสคริปต์เก่าที่ค้างอยู่ (แก้ปัญหาปิงขึ้น/แลคก่อนที่ UI จะโหลด)
if getgenv().PayomboyZ_Unload then pcall(getgenv().PayomboyZ_Unload) end
getgenv().PayomboyZ_Unload = function()
    ...
    getgenv().InventoryBalancing = false
    getgenv().SmartBase = false
    getgenv().AutoOpenPack = false
    getgenv().AutoUpgradeBase = false
    getgenv().AutoReroll = false
    getgenv().AutoRankReroll = false
    getgenv().AutoMerchant = false
    if getgenv().CardChildAddedConnection then pcall(function() getgenv().CardChildAddedConnection:Disconnect() end); getgenv().CardChildAddedConnection = nil end
    if getgenv().BruteForceLoop then pcall(function() getgenv().BruteForceLoop:Disconnect() end); getgenv().BruteForceLoop = nil end
    if getgenv().AntiAfkConnection then pcall(function() getgenv().AntiAfkConnection:Disconnect() end); getgenv().AntiAfkConnection = nil end
end

-- Multi-CDN Resilient Dependency Fetcher (Bypasses ISP / Executor Network Blocks)
local function fetchDependency(urls, name)
    for _, url in ipairs(urls) do
        for i = 1, 2 do
            local success, result = pcall(function()
                return game:HttpGet(url)
            end)
            if success and result and type(result) == "string" and #result > 100 and not result:find("404") and not result:find("Curl error") then
                local funcSuccess, func = pcall(loadstring, result)
                if funcSuccess and type(func) == "function" then
                    local execSuccess, lib = pcall(func)
                    if execSuccess and lib then
                        return lib
                    end
                end
            end
    ...
    warn("Failed to load dependency: " .. tostring(name))
    return nil
end
...
if not Fluent then
    warn("Failed to load Fluent UI from all sources! (SSL/Network Error)")
    return
end
...
local function getCardModelRarityAndMutation(model)
    ...
    local cachedRarity = model:GetAttribute("_CachedRarity")
    local cachedMutation = model:GetAttribute("_CachedMutation")
    if cachedRarity and cachedMutation then
        return cachedRarity, cachedMutation
    end
    ...
    pcall(function()
        model:SetAttribute("_CachedRarity", rarity)
        model:SetAttribute("_CachedMutation", mutation)
    end)

    return rarity, mutation
end

local function processSingleCardModel(model)
    if not getgenv().AutoBuyCards then return end
    if not model or not model:IsA("Model") or model:GetAttribute("IgnoreTutoBeam") == nil then return end
    if model:GetAttribute("Rejected") == true then return end
    ...
    if not firstSeen then
        firstSeen = tick()
        model:SetAttribute("FirstSeen", firstSeen)
    end
...
local function instantBuyLoop()
    ...
    for _, model in ipairs(getgenv().CardFolder:GetChildren()) do
        processSingleCardModel(model)
    end
end

local function setupCardFolderWatcher()
    if not getgenv().CardFolder then findCardFolder() end
    if getgenv().CardFolder then
        if getgenv().CardChildAddedConnection then
            pcall(function() getgenv().CardChildAddedConnection:Disconnect() end)
        end
        getgenv().CardChildAddedConnection = getgenv().CardFolder.ChildAdded:Connect(function(child)
            if getgenv().AutoBuyCards then
                task.spawn(function()
                    processSingleCardModel(child)
                end)
            end
        end)
    end
...
if getgenv().BruteForceLoop then pcall(function() getgenv().BruteForceLoop:Disconnect() end) end
...
local imUrls = {
    "https://cdn.jsdelivr.net/gh/1dontgiveaf/Fluent@main/Addons/InterfaceManager.lua",
    "https://fastly.jsdelivr.net/gh/1dontgiveaf/Fluent@main/Addons/InterfaceManager.lua",
    "https://raw.githubusercontent.com/1dontgiveaf/Fluent/main/Addons/InterfaceManager.lua",
    "https://statically.io/gh/1dontgiveaf/Fluent/main/Addons/InterfaceManager.lua"
}
local InterfaceManager = fetchDependency(imUrls, "InterfaceManager")

local smUrls = {
    "https://cdn.jsdelivr.net/gh/1dontgiveaf/Fluent@main/Addons/SaveManager.lua",
    "https://fastly.jsdelivr.net/gh/1dontgiveaf/Fluent@main/Addons/SaveManager.lua",
    "https://raw.githubusercontent.com/1dontgiveaf/Fluent/main/Addons/SaveManager.lua",
    "https://statically.io/gh/1dontgiveaf/Fluent/main/Addons/SaveManager.lua"
}
local SaveManager = fetchDependency(smUrls, "SaveManager")
